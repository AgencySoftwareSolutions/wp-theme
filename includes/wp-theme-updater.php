...
class WP_Theme_Updater {
	...
	private function register_hooks(): void {
		// Essential filter for WordPress update system
	...
	public function check_theme_updates( $transient ) {
		...
		// Get current theme version (fresh, not cached)
		$theme = wp_get_theme( $this->theme_slug );
		$current_version = $theme->parent() ? $theme->parent()->get( 'Version' ) : $theme->get( 'Version' );
		
		// If version changed since last check, clear cache
	...
	public function clear_transient(): void {
		...
		wp_cache_delete( 'wp_github_theme_update_' . $this->theme_slug, 'transient' );
 ...
 * @return void
 */
function wp_theme_updater(): void {
	...
	$updater = WP_Theme_Updater::getInstance( [
		'theme_slug'  => 'wp-theme',
		'github_user' => 'AgencySoftwareSolutions',
...
add_action( 'admin_init', 'wp_theme_updater' ); // Use admin_init hook that runs after $pagenow is available
